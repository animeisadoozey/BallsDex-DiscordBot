...
from ..models import BattleBuff, BattleObject
from .display import fill_battle_embed_fields
...
class BattleView(View):
    ...
    async def interaction_check(self, interaction: discord.Interaction["BallsDexBot"], /) -> bool:
        try:
            ...
            await interaction.response.send_message("You are not allowed to interact with this battle.", ephemeral=True)
            return False
        else:
            return True
    ...
    async def lock(self, interaction: discord.Interaction["BallsDexBot"], button: Button):
        ...
        if self.battle.battler1.locked and self.battle.battler2.locked:
            await interaction.followup.send(
                "Your proposal has been locked. Now confirm again to end the battle.", ephemeral=True
            )
        else:
            await interaction.followup.send(
    ...
    async def cancel(self, interaction: discord.Interaction["BallsDexBot"], button: Button):
        ...
        view = ConfirmChoiceView(
            interaction, accept_message="Cancelling the battle...", cancel_message="This request has been cancelled."
        )
        await interaction.followup.send("Are you sure you want to cancel this battle?", view=view, ephemeral=True)
        await view.wait()
        if not view.value:
            return
...
class ConfirmView(View):
    ...
    async def accept_button(self, interaction: discord.Interaction["BallsDexBot"], button: Button):
        ...
        if battler.accepted:
            await interaction.response.send_message("You have already accepted this battle.", ephemeral=True)
            return
...
class BattleMenu:
    ...
    async def start(self):
        ...
        self.message = await self.channel.send(
            content=f"Hey {self.battler2.user.mention}, {self.battler1.user.name} is proposing a battle with you!",
            embed=self.embed,
    ...
    async def perform_battle(self) -> tuple[discord.Embed, str]:
        ...
        await self.model.arefresh_from_db()
...
class CountryballsSource(ListPageSource):
    ...
    async def prepare(self):
        ...
        balls = [x async for x in BallInstance.objects.filter(id__in=first_entries)]
        for ball in balls:
...
class CountryballsSelector(Pages):
    def __init__(self, interaction: discord.Interaction["BallsDexBot"], balls: List[int], cog: BattleCog):
        self.bot = interaction.client
    ...
    async def confirm_button(self, interaction: discord.Interaction["BallsDexBot"], button: Button):
        ...
        if battle is None or battler is None:
            return await interaction.followup.send(
                "The battle has been cancelled or the user is not part of the battle.", ephemeral=True
            )
        if battler.locked:
            return await interaction.followup.send(
                "You have locked your proposal, it cannot be edited! "
        ...
        if any(ball.pk == battleball.instance.pk for ball in self.balls_selected for battleball in battler.proposal):
            return await interaction.followup.send(
        ...
        if not battle.duplicates and any(
            ball.ball_id in {b.instance.ball_id for b in battler.proposal} for ball in self.balls_selected
        ):
            await interaction.followup.send("You've selected a ball that is already in your proposal.", ephemeral=True)
            return
        ...
        await battle.model.arefresh_from_db()
        for ball in self.balls_selected:
            ...
            if battle.buffs:
                buff = await BattleBuff.objects.filter(ball=ball.countryball).afirst()
...
class BattleViewSource(ListPageSource):
...
class BattleViewMenu(Pages):
    def __init__(self, interaction: discord.Interaction["BallsDexBot"], proposal: List[BattleUser], cog: BattleCog):
        self.bot = interaction.client
    ...
    async def select_player_menu(self, interaction: discord.Interaction["BallsDexBot"], item: discord.ui.Select):
        ...
        battle_player = battle.battler2 if battle.battler2.user.id == player.discord_id else battle.battler1
        ball_instances = BallInstance.objects.filter(id__in=[x.instance.pk for x in battle_player.proposal])
        if await ball_instances.acount() == 0:
            return await interaction.followup.send(
                f"{battle_player.user} has not added any {settings.plural_collectible_name}.", ephemeral=True
            )

        view = CountryballsViewer()
        view.restrict_author(interaction.user.id)
        menu = Menu.countryballs(self.bot, view, view.selected, ball_instances)
        await menu.init(position=2)
        if battler.user.id != interaction.user.id:
            view.header.content = f"Viewing {battler.user.name}'s {settings.plural_collectible_name}"
        else:
