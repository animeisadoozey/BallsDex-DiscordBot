...
async def drop(
    interaction: discord.Interaction["BallsDexBot"], countryball: BallInstanceTransform
):
    """
    ...
    if not isinstance(channel, discord.TextChannel):
        await interaction.response.send_message(
            "This channel isn't a text channel.", ephemeral=True
        )
        return
    if not countryball.is_tradeable:
        await interaction.response.send_message(
            "The countryball must be tradeable.", ephemeral=True
        )
        return
    ...
    await interaction.followup.send(
        f"Are you sure you want to drop {description}?", view=view, ephemeral=True
    )
    await view.wait()
    if not view.value:
        return
...
async def leaderboard(
    ...
    instances = [
        {"player": players[x["player_id"]], "ball_count": x["ball_count"]}
        for x in values
    ]
